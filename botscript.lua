...
-- Handle commands sent via whisper
game:GetService("MessagingService"):SubscribeAsync(speaker.Name, function(sender, message)
	local senderUserId = tonumber(sender)
	if senderUserId == targetUserId and string.sub(message, 1, 1) == "." then
		local command = string.sub(message, 2) -- Remove the leading dot
		execCmd(command)
	end
end)
